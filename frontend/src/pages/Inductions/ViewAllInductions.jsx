...
const ViewApprovedCVs = () => {
  ...
  useEffect(() => {
    const fetchCVs = async () => {
      try {
        const response = await axiosInstance.get('http://localhost:5000/api/internships/cv/interviewpass');
  ...
  useEffect(() => {
    const fetchInterviews = async () => {
      try {
        const response = await axiosInstance.get('http://localhost:5000/api/induct');
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="flex justify-between items-center p-4">
        <h1 className="text-2xl font-semibold text-gray-800 flex items-center">
          <CheckCircleIcon className="h-6 w-6 text-green-500 mr-2" /> Assign Interns to Induction
        </h1>
      </div>
